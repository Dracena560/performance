...
export async function POST(request: Request) {
  ...
  if (form.get('grant_type') !== 'authorization_code' || typeof code !== 'string' || typeof verifier !== 'string') return NextResponse.json({ error: 'invalid_request' }, { status: 400 });
  const payload = readAuthorizationCode(code);
  if (!payload || (typeof clientId === 'string' && payload.clientId !== clientId) || (typeof redirectUri === 'string' && payload.redirectUri !== redirectUri) || payload.codeChallenge !== codeChallenge(verifier) || payload.sub !== process.env.HEALTH_GPT_USER_ID) return NextResponse.json({ error: 'invalid_grant' }, { status: 400 });
