...
const toolList = [
  ...
  { name: 'registrar_tenis', title: 'Registrar tênis', description: 'Registra treino ou jogo de tênis, inclusive dados pré e pós. Para partidas, inclua outcome (vitória ou derrota) e sets como lista de objetos {felipe:number, adversario:number}; derive também score como, por exemplo, 6-2, 6-0. Inclua tipo, adversário/parceiro, duração, energia física, clareza, motivação, foco, esforço, sintomas, análise livre e todos os dados fornecidos. Ao ler foto de calendário ou placar, extraia cada set visível.', inputSchema: { type: 'object', required: ['date','data'], properties: { date: { type: 'string' }, data: { type: 'object' }, notes: { type: 'string' } } }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false }, securitySchemes: [{ type: 'oauth2', scopes: ['health:write'] }] },
...
async function callTool(name: string, argumentsValue: unknown) {
  ...
  if (name === 'planejar_semana') { const plans=Array.isArray(args.plans)?args.plans:[]; const saved=[]; for(const raw of plans){const plan=input(raw);const date=dateFrom(plan.date),dayType=String(plan.day_type??'customizado'),targets=input(plan.targets);const result=await db.from('day_plans').upsert({user_id:process.env.HEALTH_GPT_USER_ID!,local_date:date,day_type:dayType,targets,agenda:Array.isArray(plan.agenda)?plan.agenda:[],rationale:String(plan.rationale??'')},{onConflict:'user_id,local_date'}).select().single();if(result.error)throw new Error(result.error.message);const day=await db.from('days').upsert({user_id:process.env.HEALTH_GPT_USER_ID!,local_date:date,day_type:dayType,targets},{onConflict:'user_id,local_date'});if(day.error)throw new Error(day.error.message);saved.push(result.data);} return textResult({saved:true,plans:saved}); }
