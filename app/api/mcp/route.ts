...
const toolList = [
  ...
  { name: 'registrar_refeicao', title: 'Registrar refeição', description: 'Registra uma refeição confirmada. Separe todos os ingredientes em itens individuais. Nunca envie um item apenas com nome: calorias, proteína, carboidratos, gordura, gordura saturada, fibra, açúcar e sódio são obrigatórios por 100 g. Inclua vitaminas e minerais quando puder estimá-los. Quando a porção ou nutrição vier de descrição ou foto, use a melhor estimativa e estimated=true. Use estimated=false somente com rótulo ou valor informado pelo usuário.', inputSchema: { type: 'object', required: ['name', 'items'], properties: { name: { type: 'string' }, meal_type: { type: 'string' }, items: { type: 'array', items: { type: 'object', required: ['name', 'grams', 'nutrition'], properties: { name: { type: 'string' }, grams: { type: 'number', minimum: 1 }, nutrition: { type: 'object', required: ['calories','protein','carbs','fat','saturated_fat','fibre','sugar','sodium'], properties: { calories: { type: 'number', minimum: 0, description: 'kcal por 100 g' }, protein: { type: 'number', minimum: 0, description: 'g por 100 g' }, carbs: { type: 'number', minimum: 0, description: 'g por 100 g' }, fat: { type: 'number', minimum: 0, description: 'g por 100 g' }, saturated_fat: { type: 'number', minimum: 0, description: 'g por 100 g' }, fibre: { type: 'number', minimum: 0, description: 'g por 100 g' }, sugar: { type: 'number', minimum: 0, description: 'g por 100 g' }, sodium: { type: 'number', minimum: 0, description: 'mg por 100 g' }, vitamin_a: { type: ['number','null'], description: 'mg por 100 g' }, vitamin_c: { type: ['number','null'], description: 'mg por 100 g' }, vitamin_d: { type: ['number','null'], description: 'mg por 100 g' }, vitamin_b12: { type: ['number','null'], description: 'mg por 100 g' }, folate: { type: ['number','null'], description: 'mg por 100 g' }, calcium: { type: ['number','null'], description: 'mg por 100 g' }, iron: { type: ['number','null'], description: 'mg por 100 g' }, magnesium: { type: ['number','null'], description: 'mg por 100 g' }, potassium: { type: ['number','null'], description: 'mg por 100 g' }, zinc: { type: ['number','null'], description: 'mg por 100 g' } } } } } }, hunger: { type: 'number', minimum: 0, maximum: 10 }, satiety: { type: 'number', minimum: 0, maximum: 10 }, estimated: { type: 'boolean', description: 'true para valores inferidos ou estimados; false apenas para dados confirmados.' }, occurred_at: { type: 'string' }, notes: { type: 'string' } } }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false }, securitySchemes: [{ type: 'oauth2', scopes: ['health:write'] }] },
