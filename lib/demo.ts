...
export function demoData(){
 ...
 return {healthRecords:[],periodHealthRecords:[],periodEvents:[],day:{local_date:date,day_type:'dia sem tênis · caminhada com Caju',targets:base},foods:[food],mealTemplates:[],draft:null,templates:['descanso','normal','tênis leve','tênis moderado','tênis intenso','customizado'].map((x,i)=>({id:String(i),name:x,day_type:x,targets:x.startsWith('tênis')?{...base,water:{kind:'range',min:2800,max:3500}}:base})) as TargetTemplate[],events:[
